...
function getLeaderBoard($modeJeu = 0){
    if($modeJeu == 0){
        $query = 'SELECT pseudo, score FROM Weirdle_Score JOIN Weirdle_Utilisateur ON Weirdle_Score.idUtilisateur = Weirdle_Utilisateur.idUtilisateur ORDER BY score DESC LIMIT 10';
    }else{
        $query = 'SELECT pseudo, score FROM Weirdle_Score JOIN Weirdle_Utilisateur ON Weirdle_Score.idUtilisateur = Weirdle_Utilisateur.idUtilisateur  WHERE modeJeu = :modeJeu ORDER BY score DESC LIMIT 10';
    }
